...
ssh -o ProxyCommand="ssh -i bastion_key.pem -W %h:%p -o StrictHostKeyChecking=no ubuntu@$(terraform output bastion_host_public_ip | tr -d '"')" -o StrictHostKeyChecking=no -i ec2_key.pem ubuntu@$(terraform output private_ec2_private_ip_slave1 | tr -d '"') "nohup kubectl port-forward svc/microservice1 8080:8080 --address='0.0.0.0' >/dev/null 2>&1 &"

echo "Enabling kubectl forward proxy to access kubernetes Dashboard. "
ssh -o ProxyCommand="ssh -i bastion_key.pem -W %h:%p -o StrictHostKeyChecking=no ubuntu@$(terraform output bastion_host_public_ip | tr -d '"')" -o StrictHostKeyChecking=no -i ec2_key.pem ubuntu@$(terraform output private_ec2_private_ip_slave3 | tr -d '"') "nohup kubectl port-forward -n kubernetes-dashboard service/kubernetes-dashboard 30033:443 --address='0.0.0.0' >/dev/null 2>&1 &"
